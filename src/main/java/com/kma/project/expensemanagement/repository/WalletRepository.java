...
public interface WalletRepository extends JpaRepository<WalletEntity, Long> {
    ...
    @Query(value = " select w from WalletEntity w where (w.createdBy = :createdBy) or (:groupId is not null and w.groupId = :groupId) order by w.createdAt")
    List<WalletEntity> findAllByCreatedByOrGroupIdOrderByCreatedAt(Long createdBy, Long groupId);
